...
function checkActionsOver(state) {
  ...
  if (state.state === STATE_EFFECT) {
    var effectFn = state.effectFn;
    var effectPlayer = state.effectPlayer;
    var effectCard = state.effectCard;
    var effectCardLocationList = state.effectCardLocationList;
    var effectParams = state.effectParams;
    onlyPopState(state);
    effectFn(state, effectPlayer, effectCard, effectCardLocationList, effectParams);
    checkActionsOver(state);
    return;
  }
...
DominionGame.prototype.shuffleAndDeal = function(playerAiList, seed) {
  ...
  this.effectCardLocationList = null;
...
function doPutRevealedCardsIntoHand(state, player, card, cardLocationList, params) {
  ...
  for (; i < player.revealedCards.length;) {
    ...
    if (isCardType(card, params.type)) {
